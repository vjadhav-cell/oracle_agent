...
class OracleAdapter:
    ...
    def __init__(self, tenant_id: str | None = None, connection_string: str | None = None):
        ...
        self.connection_string = connection_string or settings.DB_CONNECTION_STRING or os.getenv("DB_CONNECTION_STRING")
    ...
    def _engine_or_error(self) -> Engine:
        """Create the SQLAlchemy engine only when the first Oracle tool runs."""
    ...
    def _run_query(self, sql: str, params: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        try:
    ...
    async def get_columns(self, data: OracleGetColumnDetailsInput) -> list[dict[str, Any]]:
        ...
        sql = f"""
            SELECT
                ...
                NULLABLE
            FROM {table_source}
            WHERE {owner_filter}TABLE_NAME = :table_name
            ORDER BY COLUMN_ID
        """
        return self._run_query(sql, params)
    ...
    async def get_schema(self, data: OracleGetSchemaInput | None = None) -> dict[str, Any]:
        ...
        for table in tables:
            ...
            key = self._table_ref(owner, table_name)
            schema[key] = {
                "owner": owner,
